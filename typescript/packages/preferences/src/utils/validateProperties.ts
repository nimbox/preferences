import type { Issue, Property, Schema } from '../types.js';
...
export function validateProperty(key: string, property: Property): Issue[] {
    ...
    if (typeof property.overridable !== 'boolean') {
        errors.push(issue({
            code: IssueCode.MISSING_OVERRIDABLE,
            key,
    ...
    if (property.description !== undefined) {
        if (typeof property.description !== 'string' || property.description.length === 0) {
            errors.push(issue({
                code: IssueCode.INVALID_DESCRIPTION,
                key,
    ...
    if (property.deprecationMessage !== undefined) {
        if (typeof property.deprecationMessage !== 'string' || property.deprecationMessage.length === 0) {
            errors.push(issue({
                code: IssueCode.INVALID_DEPRECATION_MESSAGE,
                key,
...
export function validateProperties(schema: Schema): Issue[] {
...
function validateEnumShape(key: string, property: Property, errors: Issue[]): void {
    ...
    if (propertyEnum !== undefined
        ...
        errors.push(issue({
            code: IssueCode.ENUM_TYPE_MISMATCH,
            key,
    ...
    if (Array.isArray(labels)) {
        if (!Array.isArray(propertyEnum)) {
            errors.push(issue({
                code: IssueCode.ENUM_LABELS_LENGTH_MISMATCH,
                key,
        ...
        labels.forEach((label, index) => {
            if (typeof label !== 'string' || label.length === 0) {
                errors.push(issue({
                    code: IssueCode.EMPTY_ENUM_LABEL,
                    key,
    ...
    if (Array.isArray(descriptions)) {
        if (!Array.isArray(propertyEnum)) {
            errors.push(issue({
                code: IssueCode.ENUM_DESCRIPTIONS_LENGTH_MISMATCH,
                key,
        ...
        descriptions.forEach((description, index) => {
            if (typeof description !== 'string' || description.length === 0) {
                errors.push(issue({
                    code: IssueCode.EMPTY_ENUM_DESCRIPTION,
                    key,
...
function validateConstraintConsistency(key: string, property: Property, errors: Issue[]): void {

    if (typeof property.minimum === 'number' && typeof property.maximum === 'number'
        && property.minimum > property.maximum) {
        errors.push(issue({
            code: IssueCode.BOUND_INVERTED,
            key,
    ...
    if (typeof property.pattern === 'string' && property.pattern.length > 0) {
        try {
            ...
            errors.push(issue({
                code: IssueCode.INVALID_PATTERN,
                key,
...
function validateDefault(key: string, property: Property, errors: Issue[]): void {
    ...
    if (!isOfDeclaredType(propertyType, defaultValue, property.items?.type)) {
        errors.push(issue({
            code: IssueCode.INVALID_DEFAULT_TYPE,
            key,
    ...
    if (Array.isArray(property.enum)) {
        const matches = property.enum.some((member) => deepEqual(member, defaultValue));
        if (!matches) {
            errors.push(issue({
                code: IssueCode.ENUM_DEFAULT_MISMATCH,
                key,
    ...
    if (propertyType === 'integer' || propertyType === 'number') {
        ...
        if (typeof property.minimum === 'number' && numeric < property.minimum) {
            errors.push(issue({
                code: IssueCode.DEFAULT_CONSTRAINT_VIOLATION,
                key,
